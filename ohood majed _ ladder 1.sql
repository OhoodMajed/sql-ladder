-- ِ1) Query all the data in the `pets` table.  
     SELECT * FROM pets;
...
-- 4) Query the pets in the `pets` table, sorted youngest to oldest.
     SELECT * 
	 FROM pets 
	 ORDER BY age ASC; -- OR AS WE KNOW ASC IS DEFULLT
...
-- 17) Select all employees from `employees_null` where the job title is missing. What do you see?
	...
	FROM employees_null 
	WHERE job IS NULL;
-- Observation: I see a row with an ID of 101, where the firstname and lastname are both listed as 'UNKNOWN', the salary is -1, and the job column is NULL (missing). 
--This looks like a placeholder or an erroneous/corrupted data entry rather than a real employee's record.
...
-- 29) Last year, only salespeople were eligible for bonuses. Create a column `bonus` that is "Yes" if you're eligible for a bonus, otherwise "No".
    SELECT *,
    CASE 
        WHEN job = 'Sales' THEN 'Yes'  -- OR ASLO when  job like '%Sales%' then 'Yes'
        ELSE 'No' 
    END AS bonus
    FROM employees;
...
-- Create this `target_comp` column, making sure to format _both_ the `salary` and `target_comp` columns nicely (ie, with dollar signs and comma separators).
    SELECT * FROM employees;,
    PRINTF('$%,d', CAST(salary AS INTEGER)) AS salary,
    PRINTF('$%,d', CAST(
        CASE 
	...
	-- if we need with full name and no duplicate salary
SELECT 
